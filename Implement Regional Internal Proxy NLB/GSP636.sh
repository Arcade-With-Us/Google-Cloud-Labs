...
# arcadewithus : derive zones from region
...
# arcadewithus: create custom VPC + backend-subnet + proxy-only-subnet
echo "${CYAN_TEXT}${BOLD_TEXT}[TASK 1] Creating VPC network and subnets... | ${RESET_FORMAT}"

gcloud compute networks create lb-network \
  --subnet-mode=custom \
  --description="arcadewithus - GSP636 custom VPC network" \
  --quiet

echo "${GREEN_TEXT}  ✔ lb-network created${RESET_FORMAT}"

# arcadewithus: backend subnet 10.1.2.0/24
gcloud compute networks subnets create backend-subnet \
  --network=lb-network \
  --region=${REGION} \
  --range=10.1.2.0/24 \
  --description="arcadewithus - backend instances subnet" \
  --quiet

echo "${GREEN_TEXT}  ✔ backend-subnet created (10.1.2.0/24)${RESET_FORMAT}"

# arcadewithus: proxy-only-subnet — reserved for Envoy proxies, DO NOT assign backends here
gcloud compute networks subnets create proxy-only-subnet \
  ...
  --role=ACTIVE \
  --description="arcadewithus - proxy-only subnet for internal NLB Envoy proxies" \
  --quiet
...
# arcadewithus: three rules needed — SSH, GCP health check IPs, proxy-only-subnet range
echo "${CYAN_TEXT}${BOLD_TEXT}[TASK 2] Creating firewall rules... | ${RESET_FORMAT}"

# arcadewithus: allow SSH from anywhere to tagged instances
gcloud compute firewall-rules create fw-allow-ssh \
  ...
  --rules=tcp:22 \
  --description="arcadewithus - allow SSH to backend and client VMs" \
  --quiet

echo "${GREEN_TEXT}  ✔ fw-allow-ssh${RESET_FORMAT}"

# arcadewithus: GCP health checker source ranges — required for backend health probes
gcloud compute firewall-rules create fw-allow-health-check \
  ...
  --rules=tcp:80 \
  --description="arcadewithus - allow GCP health checker IPs to reach backends on port 80" \
  --quiet

echo "${GREEN_TEXT}  ✔ fw-allow-health-check${RESET_FORMAT}"

# arcadewithus: proxy-only-subnet range — Envoy proxies forward client traffic from this range
gcloud compute firewall-rules create fw-allow-proxy-only-subnet \
  ...
  --rules=tcp:80 \
  --description="arcadewithus - allow proxy-only-subnet (Envoy) traffic to backends on port 80" \
  --quiet
...
# arcadewithus: template includes Apache + hostname page + all 3 network tags
...
gcloud compute instance-templates create int-tcp-proxy-backend-template \
  ...
  --description="arcadewithus - backend template for GSP636 internal proxy NLB" \
  --metadata=startup-script='#! /bin/bash
# arcadewithus - GSP636 backend startup script
apt-get update
...
echo "Page served from: $vm_hostname | arcadewithus" | \
...
# arcadewithus: mig-a in zone-b
echo "${CYAN_TEXT}${BOLD_TEXT}[TASK 3] Creating MIG mig-a in ${ZONE_A}... | arcadewithus${RESET_FORMAT}"

gcloud compute instance-groups managed create mig-a \
  --template=int-tcp-proxy-backend-template \
  --size=2 \
  --zone=${ZONE_A} \
  --description="arcadewithus - mig-a backend group zone ${ZONE_A}" \
  --quiet

# arcadewithus: named port tcp80→80 used by backend service port-name mapping
...
# arcadewithus: mig-c in zone-c for zonal redundancy
...
gcloud compute instance-groups managed create mig-c \
  ...
  --size=2 \
  --zone=${ZONE_C} \
  --description="arcadewithus - mig-c backend group zone ${ZONE_C}" \
  --quiet
...
# ─── TASK 5: CLIENT VM ───────────────────────────────────────────────
# arcadewithus: client VM must be inside lb-network to reach internal LB VIP
...
gcloud compute instances create client-vm \
  ...
  --subnet=backend-subnet \
  --tags=allow-ssh \
  --description="arcadewithus - internal client VM to test GSP636 NLB" \
  --quiet
...
# ─── WAIT FOR BACKENDS ───────────────────────────────────────────────
# arcadewithus: wait for MIG VMs to boot and Apache startup script to complete
...
# ─── HEALTH CHECK ────────────────────────────────────────────────────
# arcadewithus: both mig-a and mig-c must show HEALTHY before testing
echo "${YELLOW_TEXT}${BOLD_TEXT}[CHECK] Backend health status: | arcadewithus${RESET_FORMAT}"
...
echo
# ─── SELF CLEANUP ────────────────────────────────────────────────────
# arcadewithus: remove this script from terminal after execution
rm -- "$0"
echo ""
