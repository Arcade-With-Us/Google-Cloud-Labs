...
echo "${CYAN_TEXT}${BOLD_TEXT}🔍 Attempting to automatically determine your default GCP zone...${RESET_FORMAT}"
export ZONE=$(gcloud compute project-info describe \
--format="value(commonInstanceMetadata.items[google-compute-default-zone])" 2>/dev/null)

if [ -z "$ZONE" ]; then
    echo "${YELLOW_TEXT}${BOLD_TEXT}⚠️ Default zone not found.${RESET_FORMAT}"
    read -p "${GREEN_TEXT}${BOLD_TEXT}Please enter the zone: ${RESET_FORMAT}" ZONE
fi

echo "${CYAN_TEXT}${BOLD_TEXT}🌍 Attempting to automatically determine your default GCP region...${RESET_FORMAT}"
...
if [ -z "$REGION" ]; then
    if [ -n "$ZONE" ]; then
        ...
        echo "${YELLOW_TEXT}${BOLD_TEXT}⚠️ Default region not found. Deriving region from zone: ${GREEN_TEXT}$REGION${RESET_FORMAT}"
    else
...
echo "${CYAN_TEXT}${BOLD_TEXT}🛠️  Creating a custom-mode VPC network named 'labnet'...${RESET_FORMAT}"
...
echo "${CYAN_TEXT}${BOLD_TEXT}🛡️  Setting up firewall rule 'labnet-allow-internal' for 'labnet' to permit ICMP and TCP port 22 (SSH) from all sources (0.0.0.0/0)...${RESET_FORMAT}"
...
echo "${CYAN_TEXT}${BOLD_TEXT}🛠️  Creating another custom-mode VPC network named 'privatenet'...${RESET_FORMAT}"
...
echo "${CYAN_TEXT}${BOLD_TEXT}🛡️  Setting up firewall rule 'privatenet-deny' for 'privatenet' to block ICMP and TCP port 22 (SSH) from all sources (0.0.0.0/0)...${RESET_FORMAT}"
